...
- `js/gemini.js` – `analyzeConversation({audio|text})` in two Gemini calls (REST `streamGenerateContent?alt=sse`, streamed so iPhones do not drop a silent request; 429/5xx and network errors retried, then fallback Flash models): **step 1** audio → numbered verbatim turns (`TRANSCRIPT_SCHEMA`, thinking low; skipped for typed text); **step 2** turns → customer + every product mentioned with status ORDERED / ASKED_ONLY / CANCELLED, sizes gathered across all its turns (`ORDER_SCHEMA`, thinking medium, full price list in the instructions). `toOrderResult` merges duplicate products, flags bad/missing sizes and lists asked-only/cancelled products (`asked`); same shape as `parseTranscript`. Default model `gemini-3.8-flash`.
